...
class User:
    ...
    def coffee_consumption(self):
        while True:
            if self.env.now >= SIMULATION_START_TIME:
                # Simulate coffee consumption in the coffee area
...
class AutonomousAgent:
    ...
    def monitor_users(self):
        while True:
            ...
            if env.now >= SIMULATION_START_TIME + 5 * 60:  # Adjust for the time of day
                for user in self.users:
                    if user.heart_rate > MESSAGE_THRESHOLD:
                        # Send a stronger message to drink water
                        if user.current_room == "Coffee Area":
                            response = random.choice(["Accept", "Ignore"])
                            if response == "Accept":
                                self.coffee_machine.display_message(user.user_id, "User took water")
                                # Record the "Take water" event
                                timestamp = datetime.fromtimestamp(env.now * 60)
                                event = {
                                    "Timestamp": timestamp,
                                ...
                                user.heart_rate += 5  # Increase heart rate when another coffee is taken
                                # Record the increased heart rate event
                                timestamp = datetime.fromtimestamp(env.now * 60)
                                event = {
                                    "Timestamp": timestamp,
