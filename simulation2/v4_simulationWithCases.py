...
class User:
    def __init__(self, env, user_id, sleep_pattern, initial_heart_rate):
        ...
        self.last_day = None  # Keep track of the last day

    def start_new_case(self):
        self.case_id += 1  # Increment case ID for a new day
        self.last_day = env.now // (24 * 60)  # Store the current day

    def move_between_rooms(self):
        while True:
            # Check if it's a new day
            if self.last_day != env.now // (24 * 60):
                self.start_new_case()
...
class CoffeeMachine:
    ...
    def display_message(self, user_id, message):
        ...
        event = {
            "Timestamp": timestamp,
            "User ID": user_id,
            "Case ID": f"Case-{user_id}-{self.users[0].case_id}",  # Include Case ID in the event log
            "Activity": message
        }
        write_event_to_csv(event)
...
plt.ylabel("Room")
plt.xticks(
    range(SIMULATION_START_TIME, SIMULATION_START_TIME + SIMULATION_END_DAYS * 24 * 60, 60 * 30),  # Set ticks every 30 minutes
    [f"{i // 60:02d}:{i % 60:02d}" for i in range(SIMULATION_START_TIME, SIMULATION_START_TIME + SIMULATION_END_DAYS * 24 * 60, 60 * 30)]
)
